...
classDiagram
    ...
    note for CoffeeFactory "咖啡工厂的抽象接口，定义创建咖啡的方法"

    %% 具体工厂
    class LatteFactory {
        +createCoffee() Coffee
    }
    note for LatteFactory "创建 Latte 实例"

    class AmericanoFactory {
        +createCoffee() Coffee
    }
    note for AmericanoFactory "创建 Americano 实例"
    ...
    %% 咖啡店（客户端）
    class CoffeeStore {
        +orderCoffee(factory: CoffeeFactory) Coffee
    }
    note for CoffeeStore "咖啡店，通过工厂订购咖啡"

    %% 客户端与工厂的关系
    CoffeeStore --> CoffeeFactory : 持有
```
